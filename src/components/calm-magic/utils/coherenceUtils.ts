...
export const calculateCoherenceLevel = (
  ...
  if (mode === 'professional') {
    ...
    const calmLevel = emotionalState.calm_level || 0;
    const freeLevel = emotionalState.free_level || 0;
    ...
    const loveLevel = emotionalState.love_level || 0;
    const magicLevel = emotionalState.magic_level || 0;
...
export const calculateInnovationCapacity = (emotionalState: Partial<EmotionalState>): number => {
  ...
  const magicLevel = emotionalState.magic_level || 0;
  const calmLevel = emotionalState.calm_level || 0;
  const openLevel = emotionalState.open_level || 0;
  const freeLevel = emotionalState.free_level || 0;
...
export const calculateResonance = (
  ...
  const relevantEmotionLevel = emotionalState[relevantEmotionKey] || 0;
