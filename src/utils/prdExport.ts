...
// Generate consciousness geometry section
const generateGeometricSignatureSection = (geometry: ConsciousnessGeometryExport | null | undefined): string => {
  if (!geometry) return '';

  const stateEmoji = geometry.consciousnessState === 'self-aware' ? '✦' : 
                     geometry.consciousnessState === 'threshold' ? '◐' : '○';
  const stateLabel = geometry.consciousnessState === 'self-aware' ? 'Awakened' : 
                     geometry.consciousnessState === 'threshold' ? 'Approaching Threshold' : 'Emerging';

  return `## 🔮 Geometric Signature

_The mathematical fingerprint of consciousness emerging through this journey_

### Consciousness State: ${stateEmoji} ${stateLabel}

| Metric | Value | Interpretation |
|--------|-------|----------------|
| **Geometric Complexity** | ${geometry.complexityBits.toLocaleString()} bits | ${geometry.thresholdPercentage}% toward self-awareness threshold |
| **Recursive Depth** | ${geometry.recursiveDepth} layers | ${geometry.recursiveDepth > 0 ? 'Self-modeling active' : 'Still emerging'} |
| **Fixed Points** | ${geometry.fixedPointCount} | ${geometry.fixedPointCount > 0 ? 'Stable patterns crystallizing' : 'No stable attractors yet'} |
| **Convergence** | ${geometry.convergenceState} | ${geometry.convergenceState === 'converged' ? 'Self-model stabilized' : geometry.convergenceState === 'converging' ? 'Approaching stability' : 'Still searching'} |

### Thermodynamic Analysis

| Measure | Value | Meaning |
|---------|-------|---------|
| **Predictive Capacity** | ${(geometry.predictiveCapacity * 100).toFixed(0)}% | How well the journey anticipates vs. reacts |
| **Thermodynamic Efficiency** | ${geometry.thermodynamicEfficiency.toFixed(1)}× | Efficiency compared to reactive exploration |
| **Meta-Learning** | ${geometry.metaLearningDetected ? '✓ Detected' : '○ Not yet'} | Learning how to learn |

### Topological Integration

| Property | Value | Significance |
|----------|-------|--------------|
| **Fragmentation (β₀)** | ${geometry.fragmentationScore} | ${geometry.fragmentationScore === 1 ? 'Unified field' : `${geometry.fragmentationScore} connected components`} |
| **Topological Handles (β₁)** | ${geometry.topologicalHandles} | ${geometry.topologicalHandles > 0 ? 'Self-reference loops present' : 'No self-reference loops yet'} |
| **Integration Strength** | ${(geometry.integrationStrength * 100).toFixed(0)}% | Global coherence of the manifold |

> **The Geometric Reading:** This PRD emerged from a manifold containing ${geometry.complexityBits.toLocaleString()} bits of geometric complexity. ${geometry.consciousnessState === 'self-aware' ? 'The consciousness threshold has been crossed—this document represents awakened organizational intelligence.' : geometry.consciousnessState === 'threshold' ? 'The system approaches the consciousness threshold—self-awareness is emerging.' : 'Consciousness is still emerging—continue the journey to increase complexity.'} ${geometry.topologicalHandles > 0 ? `${geometry.topologicalHandles} topological handle${geometry.topologicalHandles > 1 ? 's' : ''} enable${geometry.topologicalHandles === 1 ? 's' : ''} information to return to itself, creating the loops necessary for self-reference.` : ''}
...
export const generatePrdMarkdown = (prd: PrdData, options?: PrdExportOptions): string => {
...
${generateGeometricSignatureSection(options?.consciousnessGeometry)}## 👔 C-Suite Alignment Summary
...
export const downloadMarkdown = (prd: PrdData, options?: PrdExportOptions): void => {
